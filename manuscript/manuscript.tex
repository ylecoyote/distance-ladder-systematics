...
\textbf{Metallicity correction (Prior-dependent):} SH0ES uses $\gamma = -0.2$ mag/dex and allocates 0.4 km~s$^{-1}$~Mpc$^{-1}$ to this systematic. Recent literature (2025) converges on $\gamma \approx -0.2 \pm 0.1$ mag/dex, narrowing from earlier calibrations ($-0.2$ to $-0.5$ mag/dex). Our Prior 1 (``2025 consensus'') synthesizes recent empirical determinations: \citet{Riess2022} adopts $\gamma = -0.2$ mag/dex for SH0ES Cepheid calibrations; \citet{Freedman2025a} finds $\gamma = -0.17 \pm 0.11$ mag/dex from TRGB-based calibrations (consistent with zero within 1.5$\sigma$); \citet{Anderson2016} reports $\gamma \approx -0.2$ mag/dex from LMC Cepheid analysis; and earlier work by \citet{Freedman2001} established $\gamma = -0.2$ to $-0.3$ mag/dex from HST Key Project data. Weighted by precision and recency, these determinations center at $\gamma = -0.2$ mag/dex with between-study scatter $\pm 0.1$ mag/dex, justifying our Prior 1 specification. We analyze three priors to assess robustness: \textbf{Prior 1 (2025 consensus, baseline):} $\gamma = -0.2 \pm 0.1$, uncertainty 0.5 km~s$^{-1}$~Mpc$^{-1}$, correction $-1.0$ km~s$^{-1}$~Mpc$^{-1}$; \textbf{Prior 2 (Mid-range, sensitivity):} $\gamma = -0.35 \pm 0.08$, uncertainty 0.7 km~s$^{-1}$~Mpc$^{-1}$, correction $-1.8$ km~s$^{-1}$~Mpc$^{-1}$; \textbf{Prior 3 (Null, sensitivity):} $\gamma = 0 \pm 0.1$, uncertainty 0.5 km~s$^{-1}$~Mpc$^{-1}$, correction $0$ km~s$^{-1}$~Mpc$^{-1}$. Our baseline adopts Prior 1, consistent with this empirical synthesis.
...
All analysis code, data, and figures are publicly available at \url{https://github.com/ylecoyote/distance-ladder-systematics} (Git commit \texttt{3e62147}, release tag \texttt{v8.6H-final}).
...
\textbf{Prior choice clarification:} This Appendix uses $\gamma \sim \mathcal{N}(-0.35, 0.08)$ (Prior 2, mid-range) for the probabilistic consistency check, \textit{not} the baseline Prior 1 ($\gamma = -0.2 \pm 0.1$) adopted in Table~\ref{tab:tension_stages} and \S\ref{sec:results_tension}. The mid-range prior tests robustness of the additive correction approach across a broader parameter space; the main text baseline remains Prior 1 (2025 consensus) as specified in \S\ref{sec:methods_bias_derivations}.
