...
### Vibe-Check (Optional - Port Conflict Prevention)
**Use when:**
- User requests to create many worktrees in rapid succession (5+ at once)
- User mentions port conflicts or previous issues with port availability

**Example trigger**: "Create worktrees for all my feature branches (10+ branches)"

**Why**: While the slash command handles port allocation automatically, vibe-check can help with resource planning for bulk operations to prevent system resource exhaustion.

**Note**: This is optional - the `/create_worktree_prompt` command already includes robust port conflict handling and validation.
