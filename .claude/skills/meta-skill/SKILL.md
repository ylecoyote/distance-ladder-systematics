...
Claude Code provides Model Context Protocol (MCP) tools that enhance reasoning and validation. This skill benefits significantly from MCP integration during skill design.

### Sequential-Thinking
**Use when:**
- Designing a new skill with unclear scope or multiple possible approaches
- Creating complex skills with multiple supporting files and progressive disclosure
- Structuring multi-level documentation with cross-references
- Iterating on skill design after initial testing reveals issues

**Example trigger**: "Create a skill for data analysis with statistical tests and visualizations"

**Why**: Skill design requires iterative reasoning:
1. **Scope definition** (thoughts 1-3): What should be included vs excluded?
2. **Structure planning** (thoughts 4-7): How to organize instructions, files, and progressive disclosure?
3. **Instruction drafting** (thoughts 8-12): How to make steps clear and actionable?
4. **Validation and revision** (thoughts 13+): Does the skill trigger correctly? Are instructions clear?

Sequential-thinking enables you to explore design alternatives, backtrack when assumptions are wrong, and refine the skill architecture before writing final documentation.

### Vibe-Check
**Use when:**
- Before finalizing skill description (catches vague or overly broad descriptions)
- After writing instructions (validates clarity and actionability)
- Before committing to version control (ensures quality standards met)
- When testing reveals the skill doesn't trigger as expected

**Example trigger**: "Review this skill before I commit it"

**Common mistakes caught**:
- **Vague descriptions**: "helps with data" ❌ → "analyzes CSV files with pandas for statistical summaries" ✅
- **Scope creep**: Combining unrelated capabilities (e.g., "data analysis + API integration + reporting")
- **Missing trigger words**: Description doesn't include keywords users would naturally use
- **Non-actionable instructions**: Steps lack concrete commands or clear decision points
- **Missing progressive disclosure**: Dumping all content in SKILL.md instead of splitting into linked files

**Validation checklist**:
- Description includes BOTH "what it does" AND "when to use it"
- Trigger words match user's natural language
- Instructions are numbered, sequential, and actionable
- Examples cover different use cases and edge cases
- Supporting files follow progressive disclosure principles
