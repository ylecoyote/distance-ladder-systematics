...
### Vibe-Check (Safety Validation)
**Use when:**
- User requests to remove multiple worktrees (e.g., "remove all feature-* worktrees")
- User requests destructive operations without explicit confirmation
- Bulk operations that could cause port conflicts or resource issues (e.g., "create 10 worktrees")
- User provides branch name that might be a typo or doesn't exist

**Example trigger**: "Delete all my old worktrees"

**Why**: Worktree operations can be destructive and impact running services. Vibe-check provides critical safety validation to prevent:
- **Data loss**: Removing worktrees with uncommitted changes
- **Port conflicts**: Creating too many worktrees simultaneously
- **Accidental deletion**: Removing the wrong worktree due to typo
- **Service disruption**: Killing services without user awareness

**Validation checklist before destructive operations**:
- Confirm user intention for remove operations (especially bulk)
- Warn if worktree has uncommitted changes
- Validate branch names exist before creation
- Check if requested ports are already in use
- Confirm user is aware services will be stopped
